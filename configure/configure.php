...
// MENUS
function crnx_custom_theme_register_menu() {
    register_nav_menus(
        array(
...
add_action( 'init', 'crnx_custom_theme_register_menu' );
...
function crnx_deregister_scripts() {
	wp_deregister_script( 'wp-embed' );
}
add_action( 'wp_footer', 'crnx_deregister_scripts' );
...
function crnx_remove_version() {
	return '';
	}
	add_filter('the_generator', 'crnx_remove_version');


//remove Welcome Panel from WordPress dashboard
remove_action('welcome_panel', 'wp_welcome_panel');
function crnx_admin_bar_remove_logo() {
    global $wp_admin_bar;
    $wp_admin_bar->remove_menu( 'wp-logo' );
}
add_action( 'wp_before_admin_bar_render', 'crnx_admin_bar_remove_logo', 0 );
...
function crnx_limit_image_size($file) {	
...
add_filter('wp_handle_upload_prefilter', 'crnx_limit_image_size');


// add SVG to allowed file uploads
function crnx_add_file_types_to_uploads($mime_types) {
	$mime_types['svg'] = 'image/svg+xml';
	return $mime_types;
}
add_action('upload_mimes', 'crnx_add_file_types_to_uploads', 1, 1);