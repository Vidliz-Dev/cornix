...
# Get Reviews Box Custom style END
...
<span class="breadcrumbs__separator"> › </span>'; // separator between crumbs

	$before         = '

<span class="breadcrumbs__current">'; // tag before the current crumb
...
<span itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem">';
	...
	<a class="breadcrumbs__link" href="%1$s" itemprop="item">

		<span itemprop="name">%2$s</span></a>';
	...
	$blog_link      = sprintf( $link, $blog_url, $text['blog'], 1 );
	...
	if ( is_home() || is_front_page() ) {
        ...
        if ($show_home_link) {
			...
			if ( !is_category() && !is_post_type_archive() && !is_tag() && !is_author() && !has_post_format() && !is_single() ) {
				$position += 1;	
				echo $home_link;	
